...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			// Ignore non-literal values in Javascript objects.
			// @ts-expect-error -- TS2339: Property 'type' does not exist on type 'DeclarationRaws.value'.
			if (decl.parent?.type === 'object' && !['StringLiteral', 'TemplateLiteral', 'NumberLiteral'].includes(decl.raws.value?.type)) {
				return;
			}
